      ...
      //#2
      const name = inventors.map(function(person){
      ...
      //#3
      const oldYoung = inventors.sort((a, b) => a.year - b.year);
      console.log(oldYoung);

      //#4
      const totalAge = inventors.reduce((sum, age ) => {
        return sum + (age.passed - age.year);
      }, 0);
      
      console.log("The total number of year for all the inventors is: "  +  totalAge);

      //#5
      const yearsLived = inventors.sort((a, b) => (a.year - a.passed) - (b.year - b.passed));
